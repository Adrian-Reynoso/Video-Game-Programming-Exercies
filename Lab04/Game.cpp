...
bool Game::Initialize()
{
    ...
    //Create a window:
    windowPtr = SDL_CreateWindow("Mario Window", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, (int)SCREENWIDTH, (int)SCREENHEIGHT, SDL_WINDOW_OPENGL);
...
void Game::readFile(std::string fileName)
{
    ...
    for (std::string line; std::getline(filein, line); )
    {
        ...
        for (unsigned long i = 0; i < line.size(); i++)
        {
            ...
            //Add 32 to x position
            xPos += 32;
            
            //For spawning blocks
            if ((char)line[i] == 'A' || (char)line[i] == 'B' || (char)line[i] == 'C' || (char)line[i] == 'D' || (char)line[i] == 'E' || (char)line[i] == 'F' || (char)line[i] == 'G' || (char)line[i] == 'H' || (char)line[i] == 'I')
            {
            ...
            if ((char)line[i] == 'P')
            {
                ...
                mPlayer->SetPosition(Vector2(xPos, yPos));
                
                //Save pointer in member variable
                player = mPlayer;
            }
